...
static unsigned long cilksort_rand_nxt = 0;

static inline unsigned long my_rand(void) {

    cilksort_rand_nxt = cilksort_rand_nxt * 1103515245 + 12345;
    return cilksort_rand_nxt;
}

static inline void my_srand(unsigned long seed) {

    cilksort_rand_nxt = seed;
}
...
const char *cilksort_specifiers[] = {"-n", "-c", "-h", 0};
int cilksort_opt_types[] = {LONGARG, BOOLARG, BOOLARG, 0};

int cilksort_cilk_main(long n) {
    //printf("run cilk fib: %d\n", (int)n);
    ...
    //get_options(argc, argv, cilksort_specifiers, cilksort_opt_types, &size, &check, &help);
