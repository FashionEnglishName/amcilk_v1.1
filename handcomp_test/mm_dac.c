...
unsigned int mm_dac_randomSeed = 1;
...
static void rand_matrix(int *dest, int n) {
    for(int i = 0; i < n*n; ++i)
        dest[i] = rand_r(&mm_dac_randomSeed) & 0xff;
}
...
const char *mm_dac_specifiers[] = {"-n", "-c", "-h", 0};
int mm_dac_opt_types[] = {LONGARG, BOOLARG, BOOLARG, 0};
...
int mm_dac_cilk_main(int n) {
   ...
   //get_options(argc, argv, mm_dac_specifiers, mm_dac_opt_types, &size, &check, &help);
