...
static void* timer_thread_func(void * arg) {
    ...
    while (count < *life_span) {
        count++;
        printf("%d, life: %d\n", control_uid, *life_span-count);
        if (count==4 && control_uid==2) {
            period_max_s = period_max_s/4;
            printf("%d, period_max_s change: %llu\n", control_uid, period_max_s);
        }
        if (count==8 && control_uid==3) {
            period_max_s = period_max_s/4;
            printf("%d, period_max_s change: %llu\n", control_uid, period_max_s);
        }
        sprintf(str, "%d %d %d %d %d %d %llu %llu %llu %llu %llu %llu %llu %llu %f", program, input, control_uid, second_level_uid, 1, 0, period_max_s, period_max_us, period_min_s, period_min_us, C_s, C_us, L_s, L_us, elastic_coefficient); //send program run request to server
        ...
        printf("%d, period: %fs\n", control_uid, ((double)(current_time-last_time)) / 1000000000.0f);
    }
    exit(0);
    return 0;
}
...
int main(int argc, char* argv[]) {
    ...
    printf("%d, max_s %llu, max_us %llu\n", control_uid, period_max_s, period_max_us);
    ...
    while(1) {
        ...
        printf("%d, next period: %llu %llu\n", control_uid, period_s, period_us);
        if (ret_fscanf == EOF) {
            printf("End of fifo\n");
        }
        if (ret_fscanf < -1) {
