...
oldest_non_stolen_frame_in_stacklet(__cilkrts_stack_frame *head) {
    ...
    while(cur && (cur->flags & CILK_FRAME_DETACHED) == 0 && 
        ...
        //printf("TEST: stack_frame is %p , call_parent is %p\n", cur, cur->call_parent);
        cur = cur->call_parent;
        ...
        switch (t->status) {
            case CLOSURE_READY:
                ...
                if( __builtin_setjmp(w->l->rts_ctx) == 0 ) {
                    ...
                    //Zhe: here is the hook for longjmp_to_runtime
                        ...
                        if (w->l->elastic_s==ACTIVE) { //steal whole deque if has any
                            ...
                            if (w->self!=victim && victim!=-1) {
                                ...
                                if (__sync_bool_compare_and_swap(&(w->g->workers[victim]->l->elastic_s), SLEEPING_ACTIVE_DEQUE, SLEEPING_MUGGING_DEQUE)) {
                                    ...
                                    if (cl!=NULL) {
                                        if (cl->status==CLOSURE_RETURNING) {
                                            ...
                                            if (cl!=NULL) {
                                                ...
                                                //printf("1!!!!!!!!cl is null. no need to request again\n");
                                                deque_unlock_self(w);
                                            }
                                        } else if (cl->status==CLOSURE_READY) {
                                            //printf("1!!!!!!!!gugugugu[CLOSURE_READY] break\n");
                                            deque_unlock_self(w);
                                        } else if (cl->status==CLOSURE_RUNNING) {
                                            //printf("1!!!!!!!!gugugugu[CLOSURE_RUNNING] break\n");
                                            deque_unlock_self(w);
                                        } else {
                                            //printf("1!!!!!!!!gugugugu[%d]\n", cl->status);
                                            deque_unlock_self(w);
                                        ...
                                        //printf("1!!!!!!!!kakakakak[CLOSURE_NULL]???, head:%p, tail:%p\n", w->head, w->tail);
                                        deque_unlock_self(w);
                                    }
                                    //End
                                    ...
                                    //printf("\t(A: p %d)(ptr_sleeping_active_deque %d)\n", w->g->program->control_uid, w->g->elastic_core->ptr_sleeping_active_deque);
                                    elastic_core_unlock(w);
                                    //printf("steal whole deque: w is %d, victim is %d\n", w->self, victim);
                                    w->g->elastic_core->test_thief = w->self;
                        ...
                        if (w->head <= w->tail) { //the worker is set to sleep and its deque is not empty;   
                            if (__sync_bool_compare_and_swap(&(w->l->elastic_s), TO_SLEEP, SLEEPING_ADAPTING_DEQUE)) {
                                ...
                                if (__sync_bool_compare_and_swap(&(w->l->elastic_s), SLEEPING_ADAPTING_DEQUE, SLEEPING_ACTIVE_DEQUE)) {
                                    ...
                                    //be waken up
                                    if (w->head < w->tail && w->current_stack_frame!=NULL) { //never happens
                                        //printf("1\n");
                                        deque_lock_self(w);
                                        Closure *tt = deque_peek_bottom(w, w->self);
                                        //deque_unlock_self(w);//1115
                                        if (tt!=NULL) {
                                            //printf("1.1， cl_state: %d\n", tt->status);
                                            //printf("%d: buttom is %p, ptr_sleeping_inactive_deque: %d\n", w->self, tt, w->g->elastic_core->ptr_sleeping_inactive_deque);
                                            //printf("%d: sysdep_longjmp_to_sf, ptr_sleeping_inactive_deque: %d\n", w->self, w->g->elastic_core->ptr_sleeping_inactive_deque);
                                            if (__sync_bool_compare_and_swap(&(w->l->elastic_s), ACTIVATE_REQUESTED, ACTIVATING)) {
                                                ...
                                                w->g->elastic_core->ptr_sleeping_active_deque--;
                                                print_cpu_state_group(w->g->program);
                                                //printf("\t(C: p %d)(ptr_sleeping_active_deque %d)\n", w->g->program->control_uid, w->g->elastic_core->ptr_sleeping_active_deque);
                                                elastic_core_unlock(w);
                                            ...
                                            //printf("1.2\n");
                                            //printf("%d %d state %d: NULL bottom, ptr_sleeping_inactive_deque: %d, h:%p <= t:%p\n", w->g->program->control_uid, w->self, w->l->elastic_s, w->g->elastic_core->ptr_sleeping_inactive_deque, w->tail, w->head);
                                            int i = 0;
                                            for (i=0; i<w->g->options.nproc; i++) {
                                                printf("%d", w->g->workers[i]->l->elastic_s);
                                            }
                                            printf("\n");
                                            if (__sync_bool_compare_and_swap(&(w->l->elastic_s), ACTIVATE_REQUESTED, ACTIVE)) {
                                                deque_unlock_self(w);
                                                return NULL;;
                                            } else {
                                                deque_unlock_self(w);
                                    ...
                                    } else if (w->head > w->tail) { //be mugged //activated from victim of whole stolen
                                        //printf("2\n");
                                        ...
                                        if (tt!=NULL) {
                                            if (__sync_bool_compare_and_swap(&(w->l->elastic_s), ACTIVATE_REQUESTED, ACTIVATING)) { //Zhe: update
                                                ...
                                                if (__sync_bool_compare_and_swap(&(w->l->elastic_s), ACTIVATING, ACTIVE)) {
                                                    //printf("Activated!\n");
                                                    if (tt->status==CLOSURE_RUNNING) {
                                                        //printf("3.1.1\n");
                                                        deque_unlock_self(w);
                                                        sysdep_longjmp_to_sf(w->current_stack_frame);
                                                    } else if (tt->status==CLOSURE_RETURNING) {
                                                        Closure * cl = deque_xtract_bottom(w, w->self);
                                                        cl = return_value(w, cl);
                                                        if (cl!=NULL) {
                                                            printf("[FATAL]: 3.1.2.1 %d\n", tt->status);
                                                        } else {
                                                            printf("[FATAL]: 3.1.2.2 %d\n", tt->status);
                                                        }
                                                        deque_unlock_self(w);
                                                    } else {
                                                        printf("[FATAL]: 3.1.3, %d\n", tt->status);
                                                        deque_unlock_self(w);
                                                    }
                                                } else {
                                                    deque_unlock_self(w);
                                        ...
                                        } else { //be mugged //activated from victim of whole stolen
                                            if (__sync_bool_compare_and_swap(&(w->l->elastic_s), ACTIVATE_REQUESTED, ACTIVATING)) { //Zhe: update
                                                //print_cpu_state_group(w->g->program);
                                                //printf("3.2, program: %d, worker %d\n", w->g->program->control_uid, w->self);
                                                elastic_core_lock(w);
                                                elastic_do_exchange_state_group(w, w->g->workers[w->g->elastic_core->cpu_state_group[w->g->elastic_core->ptr_sleeping_inactive_deque]]);
                                                w->g->elastic_core->ptr_sleeping_inactive_deque++;
                            ...
                            if (__sync_bool_compare_and_swap(&(w->l->elastic_s), TO_SLEEP, SLEEPING_ADAPTING_DEQUE)) {
                                ...
                                if (cl!=NULL) {
                                    if (cl->status==CLOSURE_RETURNING) {
                                        ...
                                        //printf("3!!!!!!!!gugugugu[CLOSURE_READY] break\n");
                                        deque_unlock_self(w);
                                    } else if (cl->status==CLOSURE_RUNNING) {
                                        //printf("3!!!!!!!!gugugugu[CLOSURE_RUNNING] break\n");
                                        deque_unlock_self(w);
                                    } else {
                                        //printf("3!!!!!!!!gugugugu[%d]\n", cl->status);
                                        deque_unlock_self(w);
                                    ...
                                    //printf("3!!!!!!!!kakakakak[CLOSURE_NULL]???, head:%p, tail:%p\n", w->head, w->tail);
                                    deque_unlock_self(w);
                                }
                                //End
                                if (w->self==w->g->program->G->micro_time_worker_sleep_focus && w->g->program->input==15) {
                                    micro_test_get_clock(w->g->program->G, 43);//micro test
                                }
                                if (__sync_bool_compare_and_swap(&(w->l->elastic_s), SLEEPING_ADAPTING_DEQUE, SLEEPING_INACTIVE_DEQUE)) { 
                                    if (w->self==w->g->program->G->micro_time_worker_sleep_focus && w->g->program->input==15) {
                                        micro_test_get_clock(w->g->program->G, 44);//micro test
                                    }  
                                    if (w->self==w->g->program->G->micro_time_worker_sleep_focus && w->g->program->input==15) {
                                        micro_test_get_clock(w->g->program->G, 45);//micro test
                                    }  
...
void worker_sleep_handling(__cilkrts_worker *w) {
    w = __cilkrts_get_tls_worker();
    if (__sync_bool_compare_and_swap(&(w->l->elastic_s), SLEEP_REQUESTED, SLEEPING_INACTIVE_DEQUE)) {
        elastic_core_lock(w);
        w->g->elastic_core->ptr_sleeping_inactive_deque--;
        elastic_do_exchange_state_group(w, w->g->workers[w->g->elastic_core->cpu_state_group[w->g->elastic_core->ptr_sleeping_inactive_deque]]);
        elastic_core_unlock(w);
...
normal_point: //normal part, can not be preempted
        ...
        while(!t && !w->g->done) {
            ...
            if(victim_worker_id != w->self && w->l->elastic_s!=SLEEP_REQUESTED) {
                t = Closure_steal(w, victim_worker_id);
            }
