...
static void restore_x86_fp_state_for_preempt (__cilkrts_stack_frame *sf) {
    // Assume cpu supports sse
    asm volatile ("ldmxcsr %0" : : "m" (sf->prempt_mxcsr));
...
__attribute__((noreturn))
void sysdep_longjmp_to_sf_for_preempt(__cilkrts_stack_frame *sf) {
