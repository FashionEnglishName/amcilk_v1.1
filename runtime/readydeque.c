...
Closure *deque_peek_top(__cilkrts_worker *const w, int pn) {
  ...
  cl = w->g->deques[pn]->top;
  if(cl) {
    //printf("The owner_ready_deque is %d, the pn is %d.\n", cl->owner_ready_deque, pn);
    CILK_ASSERT(w, cl->owner_ready_deque == pn); //Chen
  } else {
    CILK_ASSERT(w, w->g->deques[pn]->bottom == (Closure *)NULL);
  }
...
Closure *deque_peek_bottom(__cilkrts_worker *const w, int pn) {
  ...
  cl = w->g->deques[pn]->bottom;
  if(cl) {
    //printf("TEST[%d]: pn is %d, the owner of closure is %d.\n", w->self, pn, cl->owner_ready_deque);
    //CILK_ASSERT(w, cl->owner_ready_deque == pn); //Chen: shut down assertion 
  } else {
    CILK_ASSERT(w, w->g->deques[pn]->top == (Closure *)NULL);
  }
 ...
 *       This is called only in invoke_main_slow in container-main.c.
 */
